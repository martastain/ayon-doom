...
def build_frontend(module=None):
    ...
    env = os.environ.copy()
    if module:
        env["MODULE_PATH"] = f"modules/{module}/"

    subprocess.run([yarn_executable, "install"], cwd=FRONTEND_ROOT)
    subprocess.run([yarn_executable, "build"], cwd=FRONTEND_ROOT, env=env)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="module",
        default=None,
        help="Specific frontend module to build (e.g. slicer)"
    )
    parser.add_argument(
        "--debug",
        dest="debug",
    ...
    main(args.output_dir, args.skip_zip, args.only_client, args.module)
